...
int main()
{
	...
	const uint64_t relAddr = (uint64_t)absoluteJumpMemory - ((uint64_t)funcPtr + sizeof(jmpInstruction));
	memcpy(jmpInstruction + 1, &relAddr, 4);
	memcpy(funcPtr, jmpInstruction, sizeof(jmpInstruction));

	uint64_t relAddrToRedirector = (uint64_t)absoluteJumpMemory - ((uint64_t)funcPtr + 5);
