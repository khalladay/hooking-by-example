//it's not as easy to hook 64 bit programs, because it's possible that your hook function
//is located too far away to use the 32 bit relative jump instruction. 

//This program demonstrates this by manually setting the base address of this exe to 0x100000
...
//these base addresses are set in the linker settings for each project
...
//Needs to be un-optimized, otherwise it gets optimized down to a constant in release
#pragma optimize("", off)
int GetNum()
{
	return 99;
...
{
	check(argc == 2, "Usage: basic-64bit-hook.exe <path to hook function dll>");
	...
	check(relAddr >= RELATIVE_JUMP_LIMIT || relAddr <= -RELATIVE_JUMP_LIMIT);
