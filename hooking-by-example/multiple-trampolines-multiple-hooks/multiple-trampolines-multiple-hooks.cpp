...
//thread local assembly is gnarly, so let's let the compiler handle it, we'll just call these funcs
 std::stack<uint64_t> hookJumpAddresses;
std::map<uint64_t, uint8_t*> gateFunctions;
...
void (*dogCountActionGate)(Dog*, int);
...
void InstallHook(HookDesc* hook)
{
	...
	*/
	...
	uint64_t origFuncPlus5 = (uint64_t)(hook->originalFunc) + 5;

	trampolineIter += WriteSaveArgumentRegisters(trampolineIter);	
	trampolineIter += WriteMovToRCX(trampolineIter, origFuncPlus5);
...
int main()
{
...

	{
		...
		hook.gatePtr = (void**)&dogActionGate;
	...
	}
	{
		...
		hook.gatePtr = (void**)&dogCountActionGate;
