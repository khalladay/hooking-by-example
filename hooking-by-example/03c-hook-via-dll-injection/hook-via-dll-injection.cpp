...
void printHelp()
{
	printf("DemoHookingApp-DLLInjection-Injection \nUsage: DemoHookingApp-DLLInjection-Injection <process name> <path to dll>\n");
}
...
int main(int argc, const char** argv)
{
	...
	void* dllPath = createRemoteThread(processID, argv[2]);
	...
	void* remoteFunctionToHook = FindAddressOfRemoteFunction(handle, "getNum");
	void* redirectAddr = AllocatePageNearAddressRemote(handle, remoteFunctionToHook);
	...
	void* remoteHookFunc = FindAddressOfRemoteDLLFunction(handle, argv[2], "getNumHookFunc");
	uint64_t hookFuncAddr = (uint64_t)remoteHookFunc;
	memcpy(&redirectFunc[3], &hookFuncAddr, 8);
	WriteProcessMemory(handle, redirectAddr, redirectFunc, sizeof(redirectFunc), NULL);

	//now write the E9 jump to the redirect func
	uint8_t jmp_instruction[5] = { 0xE9, 0x0, 0x0, 0x0, 0x0 };
	uint32_t relAddrToRedirector = (uint64_t)redirectAddr - ((uint64_t)remoteFunctionToHook + 5);

	memcpy(jmp_instruction + 1, &relAddrToRedirector, 4);
	WriteProcessMemory(handle, remoteFunctionToHook, jmp_instruction, sizeof(jmp_instruction), NULL);
